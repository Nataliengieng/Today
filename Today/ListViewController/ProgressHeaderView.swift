...
class ProgressHeaderView: UICollectionReusableView {
    ...
    //  an observer that updates the height constraint of the lower view when the value of progress changes
    var progress: CGFloat = 0 {
        didSet {
            setNeedsLayout()
            heightConstraint?.constant = progress * bounds.height
            UIView.animate(withDuration: 0.2) { [weak self] in
                self?.layoutIfNeeded() // force the view to update its layout immediately
            }
        }
    }
    ...
    private func prepareSubviews() {
        ...
        //  maintain a 1:1 fixed aspect ratio for the superview and container view
        ...
        //  vertically constrain the subviews
        ...
        // create an adjustable height constraint for the lower view
